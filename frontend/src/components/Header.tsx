...
function SelectWrap({
  ...
  return (
    <div className="flex flex-col gap-0.5">
      <span className="text-[10px] font-semibold text-sky-600 uppercase tracking-widest pl-0.5">{label}</span>
      <div className="relative flex items-center">
        <span className="absolute left-2.5 pointer-events-none text-sky-500">{icon}</span>
        <select
          value={value}
          onChange={(e) => onChange(e.target.value)}
          className="text-xs font-medium pl-8 pr-7 py-1.5 bg-white border border-sky-200 rounded-lg text-slate-700 focus:outline-none focus:ring-2 focus:ring-sky-400/30 focus:border-sky-400 transition-colors appearance-none cursor-pointer hover:border-sky-300 shadow-sm"
        >
          {options.map((opt) => (
    ...
    <header className="bg-sky-100 border-b border-sky-200 sticky top-0 z-50 shadow-sm">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-14 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="hidden sm:inline px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide bg-sky-200 text-sky-800 border border-sky-300 rounded-full">
                Admin
              </span>
            </div>
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-sky-200 bg-white hover:bg-sky-50 text-xs font-medium text-slate-600 transition shadow-sm"
        >
      ...
      {/* Filter bar */}
      <div className="bg-sky-50/80 border-t border-sky-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-2 flex items-end gap-4 flex-wrap">
          <div className="flex flex-col gap-0.5 flex-1 min-w-[200px]">
            <span className="text-[10px] font-semibold text-sky-600 uppercase tracking-widest pl-0.5">Search</span>
            <div className="relative flex items-center">
