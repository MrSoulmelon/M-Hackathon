      ...
      <div className="relative z-10 flex flex-col flex-1">
        <header className="bg-slate-950 border-b border-sky-800 px-6 py-4 flex justify-between items-center sticky top-0 z-50 shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-white tracking-tight">{facility?.name || 'Clinic Portal'}</h1>
        ...
        <main className="flex-1 max-w-5xl w-full mx-auto p-6 mt-4">
          <div className="flex flex-col sm:flex-row items-start sm:items-end justify-between gap-4 mb-6">
            <div>
              <h2 className="text-lg font-bold text-slate-200">Current Inventory</h2>
            ...
            <div className="flex flex-col gap-0.5 w-full sm:w-72">
              <span className="text-[10px] font-semibold text-sky-400 uppercase tracking-widest pl-0.5">Search Inventory</span>
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="text-sm text-slate-300 divide-y divide-slate-800">
                {filteredInventory.map(item => {
                  const med = medicines.find(m => m.medicine_id === item.medicine_id);
                  return (
                    <tr key={item.medicine_id} className="hover:bg-slate-950/50 transition">
                      <td className="p-4 px-6 font-semibold text-white">{med?.name || item.medicine_id}</td>
                      <td className="p-4 px-6">
                        <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-blue-950 text-blue-400 font-semibold border border-blue-900">
                      ...
                      <td className="p-4 px-6 text-right">
                        <button 
                          ...
                          className="text-blue-400 hover:text-white font-semibold text-xs px-4 py-2 border border-blue-800 hover:border-blue-600 bg-slate-900 hover:bg-blue-600 rounded-xl transition shadow-sm"
                        >
                          Update Stock
                        </button>
                      </td>
